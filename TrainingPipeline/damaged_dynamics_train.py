...
from TrainingPipeline.model.damaged_dn_transEncDec import DamagedDynamicsEncoderDecoder 
# from TrainingPipeline.model.damaged_dn_model import DamagedDynamicsTDecoder 
from TrainingPipeline.model.damaged_dn_dataloader import DamagedDyamicsFnDataset, collate_damaged_dn_data_full_t
...
class Learner:
    def __init__(self, cfg_dir: str, encoder_cfg_dir):
        ...
        self.encoder_cfg = get_conf(encoder_cfg_dir)

        self.logger = init_logger(self.cfg)
        self.logger.log_code(file_name="TrainingPipeline/model/damaged_dn_model.py")
        self.logger.log_code(file_name="TrainingPipeline/model/damaged_dn_dataloader.py")
    ...
    def train(self):
        ...
        text_embedding = data["text_embedding"].to(device=self.device)
        ...
        # pdb.set_trace()
        macs, params = op_counter(self.model, sample=(history, action, text_embedding))
    ...
    def init_dataloader(self):
        ...
        full_dataset = DamagedDyamicsFnDataset(self.cfg.damaged_dynamics)
        ...
        train_loader = DataLoader(train_dataset, collate_fn=collate_damaged_dn_data_full_t, **self.cfg.dataloader)
        ...
        val_loader = DataLoader(val_dataset, collate_fn=collate_damaged_dn_data_full_t, **val_loader_kwargs)
    ...
    def init_model(self):
        ...
        #load the model
        # model = DamagedDynamicsTDecoder(self.cfg, self.encoder_cfg, self.device)
        model = DamagedDynamicsEncoderDecoder(self.cfg, self.encoder_cfg, self.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--conf", default="config/damaged_model_config.yaml", type=str)
    parser.add_argument("--encoder_conf", default="config/pre_train_config.yaml")
    args = parser.parse_args()
    cfg_path = args.conf
    encoder_cfg_path = args.encoder_conf
    print(cfg_path, encoder_cfg_path)
    learner = Learner(cfg_path, encoder_cfg_path)
    learner.train()