...
from TrainingPipeline.model.anycar_models import AnyCarTransformerDecoder
from TrainingPipeline.model.transformer_dataloader import JaxDyamicsFnDataset, collate_transformer_data
...
class Learner:
    def __init__(self, cfg_dir: str):
        ...
        self.logger.log_code(file_name="TrainingPipeline/model/anycar_models.py")
        self.logger.log_code(file_name="TrainingPipeline/model/transformer_dataloader.py")
    ...
    def train(self):
        """Trains the model"""
        ref_model = None
        if hasattr(self.cfg.directory, 'finetune_from') and self.cfg.directory.finetune_from:
            import copy
            ref_model = copy.deepcopy(self.model)
            ref_model.eval()
            for p in ref_model.parameters():
                p.requires_grad = False
            print("  -> Reference model frozen for parameter constraint.")
        ...
        action = data["future_action"].to(device=self.device)
        gt_sequence = data["gt_sequence"].to(device=self.device)
        action_padding_mask = data["action_padding_mask"].to(device=self.device).bool()

        # pdb.set_trace()
        macs, params = op_counter(self.model, sample=(history, action))
    ...
    def forward_batch(self, data):
        ...
        self.model.train()
        history = data["history"].to(device=self.device)       #B, H,  
        action = data["future_action"].to(device=self.device) 
        gt_sequence = data["gt_sequence"].to(device=self.device) 
        action_padding_mask = data["action_padding_mask"].to(device=self.device)
        history_padding_mask = data["history_padding_mask"].to(device=self.device) 

        # pdb.set_trace() 
        predicted_pose_dot = self.model(history, 
                                        action,
                                        history_padding_mask=history_padding_mask,
                                        action_padding_mask=action_padding_mask,
                                        tgt_mask=None)
        # predicted_pose_dot = self.model(delta)
        loss = self.criterion(predicted_pose_dot, gt_sequence)
    ...
    def init_dataloader(self):
        ...
        full_dataset = JaxDyamicsFnDataset(self.cfg.anycar_transformer)
        ...
        train_loader = DataLoader(train_dataset, collate_fn=collate_transformer_data, **self.cfg.dataloader)
        ...
        val_loader = DataLoader(val_dataset, collate_fn=collate_transformer_data, **val_loader_kwargs)
    ...
    def if_resume(self):
        if self.cfg.logger.resume:
        ...
        elif hasattr(self.cfg.directory, 'finetune_from') and self.cfg.directory.finetune_from:
            # pdb.set_trace()
            print(f"{datetime.now():%Y-%m-%d %H:%M:%S} - FINE-TUNING from pre-trained model!")
            checkpoint_path = self.cfg.directory.finetune_from
            ...
            print(f"Loading Successful, {self.cfg.directory.finetune_from} is loaded")
            ...
            finetune_lr = self.cfg.adamw.lr * 0.1
            print(f"  -> Resetting optimizer with Fine-Tune LR: {finetune_lr}")
    ...
    def init_model(self):
        ...
        #load the model
        model = AnyCarTransformerDecoder(self.cfg, self.device)
        # model = ResisualBicycleDynamicsModel(self.cfg, self.del_pose_mean, self.del_pose_std, self.action_mean, self.action_std)
