...
class DamagedDyamicsFnDataset(Dataset):
    '''get items returns
    ...
    def __init__(self, cfg):
        ...
        for file_idx, rec in enumerate(self.records):
            ...
            for seq_idx in range(num_sequences):
                # Make sure there are still self.prediction_horizon length remaining, if not
                # drop the data point
    ...
    def __getitem__(self, idx):
        ...
        del_pose_noise_norm, action_noise_norm = self.apply_noise(del_pose_norm, action_norm)
        ...
        if self.dof_3_only:
            del_pose_dof  = del_pose_T[:, [0, 1, 5]]
            gt_pose_dof = gt_pose_norm[:, [0, 1, 5]]
        else:
            del_pose_dof  = del_pose_T
            gt_pose_dof = gt_pose_norm

        full_sequence_normalized = torch.cat([del_pose_dof, action_T], dim=1)   #returnee
    ...
    def apply_noise(self, del_pose, action):
        if self.add_noise:
            del_pose[:, 0] += torch.rand_like(del_pose[:, 0]) * 0.01 - 0.005
            del_pose[:, 1] += torch.rand_like(del_pose[:, 1]) * 0.01 - 0.005
            del_pose[:, 3] += torch.rand_like(del_pose[:, 3]) * 0.01 - 0.005
            del_pose[:, 4] += torch.rand_like(del_pose[:, 4]) * 0.01 - 0.005
            del_pose[:, 5] += torch.rand_like(del_pose[:, 5]) * 0.01 - 0.005
            
            ## actions
            action[:, 0] += torch.rand_like(action[:, 0]) * 0.01 - 0.005
            action[:, 1] += torch.rand_like(action[:, 1]) * 0.01 - 0.005
        
        return del_pose, action
      

def collate_damaged_dn_data_crossformer(batch):
...
def collate_damaged_dn_data_full_t(batch):
    """Collate into
    history: floatTensor [B, H, 8]
    action : floatTensor [B, F, 2]
    gt_pose : floatTensor [B, F, 6]
