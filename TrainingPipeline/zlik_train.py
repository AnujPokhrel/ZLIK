...
from TrainingPipeline.model.cross_models.cross_damaged_former import DamagedCrossformer 
from TrainingPipeline.model.damaged_dn_dataloader import DamagedDyamicsFnDataset, collate_damaged_dn_data_crossformer
...
class Learner:
    def __init__(self, cfg_dir: str, encoder_cfg_dir):
        ...
        if self.cfg.logger.resume and self.cfg.logger.experiment_key != "":
        ...
        self.logger.log_code(file_name="TrainingPipeline/model/cross_models/cross_damaged_former.py")
        self.logger.log_code(file_name="TrainingPipeline/model/cross_models/cross_former.py")
    ...
    def train(self):
        ...
        future_actions = data["future_action"].to(device=self.device)
        text_embedding = data["text_embedding"].to(device=self.device)
        gt_sequence = data["gt_sequence"].to(device=self.device)

        macs, params = op_counter(self.model, sample=(history, future_actions, text_embedding))
    ...
    def forward_batch(self, data):
        ...
        history[:, :-1, :] = float(0.0)
        history[:, :, 6:] = float(0.0)
        predicted_poses = self.model(
            history=history,
            future_actions=future_actions,
            damage_embedding=text_embedding
        )
        # pdb.set_trace()
        loss = self.criterion(predicted_poses, gt_sequence)
    ...
    def init_dataloader(self):
        ...
        train_loader = DataLoader(train_dataset, collate_fn=collate_damaged_dn_data_crossformer, **self.cfg.dataloader)
        ...
        val_loader = DataLoader(val_dataset, collate_fn=collate_damaged_dn_data_crossformer, **val_loader_kwargs)
    ...
    def init_model(self):
        ...
        #load the model
        model = DamagedCrossformer(self.encoder_cfg, self.cfg, self.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--conf", default="TrainingPipeline/conf/damaged_model_config.yaml", type=str)
